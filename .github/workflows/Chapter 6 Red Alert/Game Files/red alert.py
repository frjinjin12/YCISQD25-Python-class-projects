...
def draw():
    ...
    if game_over:
        ...
        for star in stars:
            star.draw()  # <-- Added parentheses here
...
def make_stars(number_of_extra_stars):
    ...
    animate_stars(new_stars)  # <-- Changed from layout_stars
    return new_stars
...
def create_stars(colors_to_create):
    new_stars = []  # <-- Properly initialized here
...
def layout_stars(stars_to_layout):
    ...
    for index, star in enumerate(stars_to_layout):
        ...
        star.x = new_x_pos
        star.y = 0  # Optional: Set starting Y position

def animate_stars(stars_to_animate):  # <-- Renamed from layout_stars
...
def on_mouse_down(pos):
    ...
    for star in stars:  # <-- Corrected loop variable
